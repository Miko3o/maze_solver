...
class Test_SolveMazeDijkstra(unittest.TestCase):
  ...
  def test_MazePathMade_When_SolveCalled(self, mock_pygameEventGet, mock_pygameDisplayUpdate, mock_pygameDrawRect):
    ...
    solveMazeDijkstra = SolveMazeDijkstra(mock_viewManager, mock_gameWindow)
    N = go.NOTHING
    W = go.WALL
    S = go.SOLVER
    G = go.GOAL
    ...
    #WORK------------------------------------------------
    solveMazeDijkstra.Solve(currentGrid, [], 0)
  ...
  def test_SolverLocationIsFound_When_GetSolverLocationCalled(self):
    ...
    #WORK-------------------------------------------
    solverLocation = solveMazeDijkstra.GetSolverLocation(currentGrid)
